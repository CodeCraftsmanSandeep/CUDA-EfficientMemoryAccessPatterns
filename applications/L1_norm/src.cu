...
long long int solve(const unsigned int n, const int* __restrict__ C, const int* __restrict__ B)
{
    ...
    long long int* d_L1_norm;
    cudaMalloc(&d_L1_norm, sizeof(long long int));
    ...
    long long int L1_norm;
    cudaMemcpy(&L1_norm, d_L1_norm, sizeof(long long int), cudaMemcpyDeviceToHost);
