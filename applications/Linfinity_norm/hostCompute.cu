...
void compute(const unsigned int n, const int* __restrict__ C, const int* __restrict__ B, int* __restrict__ res)
{
    int Linfinity_norm = 0;
    for(int i = 0; i < n; i++)
    {
        Linfinity_norm = max(Linfinity_norm, abs(C[i] - B[i]));
    }
    *res = Linfinity_norm;
}

int solve(const unsigned int n, const int* __restrict__ C, const int* __restrict__ B)
{
    double start_time, end_time, time_consumed;
    int* Linfinity_norm = (int*) malloc(sizeof(int));

    for(int run = 1; run <= NUM_RUNS; run++)
    {
        start_time = rtClock();
        compute(n, C, B, Linfinity_norm);
        end_time   = rtClock();
...
int main(const int argc, char* argv[]){
    ...
    // Solving
    int Linfinity_norm = solve(nC, C, B);

    // Printing results
    {
        fprintf(OUTPUT_FILE, "n,Linfinity_norm,mean-time(ms),median-time(ms),std-deviation(ms)\n");
        fprintf(OUTPUT_FILE, "%d,%d,%.6f,%.6f,%.6f\n", nC, Linfinity_norm, findMean(execution_times), findMedian(execution_times), findStandardDeviation(execution_times));
    }
