...
int solve(const unsigned int n, const int* __restrict__ C, const int* __restrict__ B)
{
    ...
    int* d_Linfinity_norm;
    cudaMalloc(&d_Linfinity_norm, sizeof(int));
    ...
    {
        start_time = rtClock();
        compute(n, d_C, d_B, d_Linfinity_norm);
        end_time   = rtClock();
    ...
    int Linfinity_norm;
    cudaMemcpy(&Linfinity_norm, d_Linfinity_norm, sizeof(int), cudaMemcpyDeviceToHost);

    cudaFree(d_C);
    cudaFree(d_B);
    cudaFree(d_Linfinity_norm);
