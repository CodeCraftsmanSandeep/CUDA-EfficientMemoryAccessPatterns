...
__global__ void blockStrideKernel (const unsigned int N, const int* __restrict__ C, const int* __restrict__ B, long long int* __restrict__ intermediary_res)
{
...
__global__ void sqrtKernel (float* __restrict__ L2_norm, long long int* __restrict__ intermediary_res)
{
    *L2_norm = sqrt((double)*intermediary_res);
}

void compute(const unsigned int N, const int* __restrict__ d_C, const int* __restrict__ d_B, long long int* __restrict__ d_intermediary_res, float* __restrict__ d_L2_norm)
{
    cudaMemset(d_intermediary_res, 0LL, sizeof(long long int));
    ...
    if (N < block_size_256 * min_work_per_thread)
    {
        blockStrideKernel       <block_size_256_power> 
                <<< 1, block_size_256, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_intermediary_res);
    }
    else if (N < num_blocks_256 * block_size_256 * min_work_per_thread) 
    {
        blockStrideKernel       <block_size_256_power>
                <<< (N >> block_size_256_power) / min_work_per_thread, block_size_256, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_intermediary_res);
    }
    else if (N < num_blocks_256 * block_size_512 * min_work_per_thread) 
    {
        blockStrideKernel       <block_size_256_power> 
               <<< num_blocks_256, block_size_256, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_intermediary_res);
    }
    else
    {
        blockStrideKernel       <block_size_512_power> 
              <<< num_blocks_256, block_size_512, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_intermediary_res);
    }
    sqrtKernel <<<1,1>>> (d_L2_norm, d_intermediary_res);
