...
void compute(const unsigned int n, const int* __restrict__ d_C, const int* __restrict__ d_B, long long int* __restrict__ d_temp_storage, float* __restrict__ d_L1_norm);
...
float solve(const unsigned int n, const int* __restrict__ C, const int* __restrict__ B)
{
    ...
    {
        start_time = rtClock();
        compute(n, d_C, d_B, d_temp_storage, d_L2_norm);
        end_time   = rtClock();
