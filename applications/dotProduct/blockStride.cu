...
__global__ void blockStrideKernel (const unsigned int N, const int* __restrict__ C, const int* __restrict__ B, long long int* __restrict__ dotProduct)
{
    ...
    while(__builtin_expect(iter < end, 1)){
        partial_sum += (long long int)C[iter] * B[iter];
        iter += block_stride;        
    }

    // Remaining work-items
    iter = ((work_per_block * gridDim.x) + global_thread_id);
    if(iter < N) partial_sum += (long long int)C[iter] * B[iter];
    ...
    if (warp_id == 0){
        ...
        if (lane_id == 0) atomicAdd(dotProduct, partial_sum);
    }
}
...
{
    cudaMemset(d_dotProduct, 0LL, sizeof(long long int));

    // Kernel invocation
    if (N < block_size_256 * min_work_per_thread)
    {
        blockStrideKernel       <block_size_256_power> 
                <<< 1, block_size_256, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_dotProduct);
    }
    else if (N < num_blocks_256 * block_size_256 * min_work_per_thread) 
    {
        blockStrideKernel       <block_size_256_power>
                <<< (N >> block_size_256_power) / min_work_per_thread, block_size_256, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_dotProduct);
    }
    else if (N < num_blocks_256 * block_size_512 * min_work_per_thread) 
    {
        blockStrideKernel       <block_size_256_power> 
               <<< num_blocks_256, block_size_256, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_dotProduct);
    }
    else
    {
        blockStrideKernel       <block_size_512_power> 
              <<< num_blocks_256, block_size_512, warp_size * sizeof(long long int) >>> (N, d_C, d_B, d_dotProduct);
    }
