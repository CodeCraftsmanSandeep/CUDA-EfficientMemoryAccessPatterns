// Takes two file paths as command line arguments and does the computation from the host
...
void compute(const unsigned int n, const int* __restrict__ C, const int* __restrict__ B, long long int* __restrict__ res)
{
    long long int dotProduct = 0;
    for(int i = 0; i < n; i++)
    {
        dotProduct += (long long int)C[i] * B[i];
    }
    *res = dotProduct;
}
...
long long int solve(const unsigned int n, const int* __restrict__ C, const int* __restrict__ B)
{
    ...
    long long int* dotProduct = (long long int*) malloc(sizeof(long long int));

    for(int run = 1; run <= NUM_RUNS; run++)
    {
        start_time = rtClock();
        compute(n, C, B, dotProduct);
        end_time   = rtClock();
    ...
    long long int final_result = *dotProduct;
    free(dotProduct);
