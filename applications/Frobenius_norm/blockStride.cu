...
__global__ void blockStrideKernel (const unsigned int matrix_size, const int* __restrict__ C, long long int* __restrict__ intermediary_res)
{
    const unsigned int global_thread_id     =   (threadIdx.x + (blockIdx.x << block_size_power));
    const unsigned int num_chuncks          =   (matrix_size >> block_size_power);
...
void compute(const unsigned int M, const unsigned int N, const int* __restrict__ d_C, long long int* __restrict__ d_intermediary_res, float* __restrict__ d_Frobenius_norm)
{
    ...
    if (matrix_size < block_size_256 * min_work_per_thread)
    {
        blockStrideKernel       <block_size_256_power> 
                <<< 1, block_size_256, warp_size * sizeof(long long int) >>> (matrix_size, d_C, d_intermediary_res);
    }
    else if (matrix_size < num_blocks_256 * block_size_256 * min_work_per_thread) 
    {
        blockStrideKernel       <block_size_256_power>
                <<< (matrix_size >> block_size_256_power) / min_work_per_thread, block_size_256, warp_size * sizeof(long long int) >>> (matrix_size, d_C, d_intermediary_res);
    }
    else if (matrix_size < num_blocks_256 * block_size_512 * min_work_per_thread) 
    {
        blockStrideKernel       <block_size_256_power> 
               <<< num_blocks_256, block_size_256, warp_size * sizeof(long long int) >>> (matrix_size, d_C, d_intermediary_res);
    }
    else
    {
        blockStrideKernel       <block_size_512_power> 
              <<< num_blocks_256, block_size_512, warp_size * sizeof(long long int) >>> (matrix_size, d_C, d_intermediary_res);
    }
    sqrtKernel <<<1,1>>> (d_Frobenius_norm, d_intermediary_res);
