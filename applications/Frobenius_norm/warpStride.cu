...
__global__ void warpStrideKernel(const unsigned int matrix_size, const int* __restrict__ C, long long int* __restrict__ intermediary_res)
{
    ...
    const unsigned int num_chuncks         =    (matrix_size >> warp_size_power);
...
__global__ void sqrtKernel (float* __restrict__ Frobenius_norm, const long long int* __restrict__ intermediary_res)
{
