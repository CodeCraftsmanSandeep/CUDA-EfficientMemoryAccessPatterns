...
template <const int block_size_power>
__global__ void treeReductionWarpStrideUnrolled16Kernel (const unsigned int N, const int* __restrict__ a, long long int* __restrict__ total_sum)
{
    const unsigned int global_thread_id    =    ((blockIdx.x << block_size_power) + threadIdx.x);
    const unsigned int global_warp_id      =    (global_thread_id >> warp_size_power);
    ...
    int iter                               =    ((work_per_warp * global_warp_id) + (threadIdx.x & 31));
    const unsigned int end                 =    (iter + work_per_warp);
    constexpr unsigned int warp_stride     =    warp_size;
    const unsigned int end_unrolled        =    end - 15 * warp_stride;   // note: the value should be non-negative  

    // Warp-stride access 
    long long int partial_sum = 0;
    while(__builtin_expect(iter < end_unrolled, 1)){
        partial_sum += (long long int)a[iter]                    + (long long int)a[iter + warp_stride]      + (long long int)a[iter + 2 * warp_stride]  + a[iter + 3 * warp_stride];
        partial_sum += (long long int)a[iter + 4 * warp_stride]  + (long long int)a[iter + 5 * warp_stride]  + (long long int)a[iter + 6 * warp_stride]  + a[iter + 7 * warp_stride];
        partial_sum += (long long int)a[iter + 8 * warp_stride]  + (long long int)a[iter + 9 * warp_stride]  + (long long int)a[iter + 10 * warp_stride] + a[iter + 11 * warp_stride];
        partial_sum += (long long int)a[iter + 12 * warp_stride] + (long long int)a[iter + 13 * warp_stride] + (long long int)a[iter + 14 * warp_stride] + a[iter + 15 * warp_stride];
        iter += (16 * warp_stride);
    }
    while(iter < end){
        partial_sum += a[iter];
...
void computeReduction(const unsigned int N, const int* d_a, long long int* d_sum, long long int init)
{
    ...
    {
        treeReductionWarpStrideUnrolled16Kernel       <block_size_256_power>
    ...
    {
       treeReductionWarpStrideUnrolled16Kernel       <block_size_512_power> 
